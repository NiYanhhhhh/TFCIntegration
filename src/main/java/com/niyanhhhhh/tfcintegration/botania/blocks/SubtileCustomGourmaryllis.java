package com.niyanhhhhh.tfcintegration.botania.blocks;
...
// copied from class SubTileGourmaryllis and modified to adapt to TFC food
public class SubtileCustomGourmaryllis extends SubTileGenerating {
    ...
    public void onUpdate() {
        ...
        for(EntityItem item : items) {
            ...
            if(!stack.isEmpty() && stack.getItem() instanceof ItemFood && !item.isDead && item.getAge() >= slowdown) {
                if(cooldown <= 0) {
                    ...
                    System.out.println("hunger: " + val);

                    // this part is added for adapting tfc foods
                    if (stack.getItem() instanceof ItemFoodTFC) {
                        IFood foodCap = (IFood) ((ItemFoodTFC) stack.getItem()).getCustomFoodHandler();
                        val = Math.min(12, foodCap.getData().getHunger());
                    }
                    System.out.println("hunger_real: " + val);
